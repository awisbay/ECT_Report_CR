...
def generate_script(file_path=None, selected_file=None, output_dir=None, all_df=None , sheet_list=None):
    ...
    cr_file_name =  re.search(r"(ExternalGNBCUCPFunction.*?|ExternalGNBCUCPFunction)\_[0-9]{8}", selected_file, re.IGNORECASE).group(1) if re.search(r"(ExternalGNBCUCPFunction.*?|ExternalGNBCUCPFunction)\_[0-9]{8}", selected_file, re.IGNORECASE).group(1) else f"{selected_file}"
    number_cr = "99"
...
"""
    ...
    #############################################
        ...
        if (sheet_name.upper() == "DEL_ExternalGNBCUCPFunction".upper()):        
            ##sheet = item_sheet 
    ...
    #############################################
    #############################################
        if (sheet_name.upper() == "ADD_ExternalGNBCUCPFunction".upper()):            
            ##sheet = item_sheet 
            ##df = pd.read_excel(io=file_name, sheet_name=sheet)
            row_count = len(df)
            column_count = len(df.columns)
    ...
    #############################################
    #############################################
        if (sheet_name.upper() == "ADD_ExternalNRCellCU".upper()):            
            ##sheet = item_sheet 
            ##df = pd.read_excel(io=file_name, sheet_name=sheet)
            row_count = len(df)
            column_count = len(df.columns)
    ...
    #############################################
    #############################################
        if (sheet_name.upper() == "ADD_ExternalNRCellRelation".upper()):            
            ##sheet = item_sheet 
            ##df = pd.read_excel(io=file_name, sheet_name=sheet)
            row_count = len(df)
            column_count = len(df.columns)    
